...
public class Pawn extends Piece {
    ...
    public ArrayList<Square> getLegalMoves(Board b, Square start) {
        ...
        if ((getColor() && row == 1) || (!getColor() && row == 6)) {
        ...
        if (col > 0 && board[row + direction][col - 1].isOccupied() && board[row + direction][col - 1].getOccupyingPiece().getColor() != getColor()) {
            legalMoves.add(board[row + direction][col - 1]);
        }
        if (col < 7 && board[row + direction][col + 1].isOccupied() && board[row + direction][col + 1].getOccupyingPiece().getColor() != getColor()) {
            legalMoves.add(board[row + direction][col + 1]);
        }
